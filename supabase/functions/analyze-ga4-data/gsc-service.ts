...
export async function analyzeGSCData(propertyUrl: string, accessToken: string): Promise<GSCAnalysis | null> {
  if (!propertyUrl) {
    console.log('No GSC property provided, skipping GSC analysis');
    return null;
  }

  console.log('Starting GSC data analysis for property:', propertyUrl);
  
  try {
    const now = new Date();
    const data = await fetchGSCData(propertyUrl, accessToken, now, now);
